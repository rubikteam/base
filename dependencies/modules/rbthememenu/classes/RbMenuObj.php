...
{ 
    ...
    {        
        ...
        if($previousId > 0)
        {
            if (Tools::ucfirst($this->fields['form']['name']) == 'Menu') {
                $objName = 'RbMegaMenu';
            } else {
    ...
    public function update($null_value=false)
    {
        ...
        if(get_class($this)=='RbMegaMenu' && $this->enabled_vertical)
        {
            $columns= Db::getInstance()->executeS('SELECT id_column FROM '._DB_PREFIX_.'rbthememenu_column WHERE id_menu='.(int)$this->id.' AND id_tab not in (SELECT id_tab FROM '._DB_PREFIX_.'rbthememenu_tab where id_menu ='.(int)$this->id.')');
            if($columns)
            {
                $id_tab= Db::getInstance()->getValue('SELECT id_tab FROM '._DB_PREFIX_.'rbthememenu_tab where id_menu='.(int)$this->id);
                if(!$id_tab)
                {
                    $tab=new RBMenuTab();
                    $tab->id_menu=$this->id;
                ...
                {
                    Db::getInstance()->execute('UPDATE '._DB_PREFIX_.'rbthememenu_column SET id_tab="'.(int)$id_tab.'" WHERE id_column='.(int)$column['id_column']);
                }
            }
        }
